...
SRC_URI_append_qemux86-64 = " https://github.com/${ANKAIOS_GITHUB_REPO}/releases/download/${ANKAIOS_RELEASE_TAG}/ankaios-linux-amd64.tar.gz;name=bin-amd64"
SRC_URI_append = " ${@' https://github.com/${ANKAIOS_GITHUB_REPO}/releases/download/${ANKAIOS_RELEASE_TAG}/ankaios-linux-arm64.tar.gz;name=bin-arm64' if d.getVar('MACHINE') in ('raspberrypi4-64', 'raspberrypi5') else ''}"
...
# The binaries are only unpacked into WORKDIR
S = "${WORKDIR}"
...
RDEPENDS_${PN} = "ank-agent-bin ank-bin ank-server-bin"

CONFFILES_ank-bin = "${sysconfdir}/ankaios/ank.conf"
CONFFILES_ank-server-bin = "${sysconfdir}/ankaios/state.yaml ${sysconfdir}/ankaios/ank-server.conf"
CONFFILES_ank-agent-bin = "${sysconfdir}/ankaios/ank-agent.conf"

SYSTEMD_PACKAGES = "${@bb.utils.contains('DISTRO_FEATURES', 'systemd', 'ank-server-bin ank-agent-bin', '', d)}"
SYSTEMD_SERVICE_ank-server-bin = "ank-server.service"
SYSTEMD_SERVICE_ank-agent-bin = "ank-agent.service"
SYSTEMD_AUTO_ENABLE = "enable"

INITSCRIPT_PACKAGES = "${@bb.utils.contains('DISTRO_FEATURES', 'sysvinit', 'ank-server-bin ank-agent-bin', '', d)}"
INITSCRIPT_NAME_ank-server-bin = "ank-server"
INITSCRIPT_PARAMS_ank-server-bin = "start 70 2 3 4 5 . stop 30 0 1 6 ."
INITSCRIPT_NAME_ank-agent-bin = "ank-agent"
INITSCRIPT_PARAMS_ank-agent-bin = "start 71 2 3 4 5 . stop 29 0 1 6 ."
...
do_install() {
    ...
    if ${@bb.utils.contains('DISTRO_FEATURES', 'systemd', 'true', 'false', d)}; then
        ...
        install -m 0644 ${WORKDIR}/ank-server.service ${D}${systemd_system_unitdir}/
        install -m 0644 ${WORKDIR}/ank-agent.service ${D}${systemd_system_unitdir}/
    fi

    if ${@bb.utils.contains('DISTRO_FEATURES', 'sysvinit', 'true', 'false', d)}; then
        install -d ${D}${sysconfdir}/init.d
        install -m 0755 ${WORKDIR}/init-scripts/ank-server ${D}${sysconfdir}/init.d/ank-server
        install -m 0755 ${WORKDIR}/init-scripts/ank-agent ${D}${sysconfdir}/init.d/ank-agent
    fi
}
